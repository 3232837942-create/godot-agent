...
(function(){
 ...
 const shotFX=new FerdinandFireEffects(world);
 let view='iso',motion='',lastMotion='',clock=0,exploded=0,wantExplode=0,last=performance.now(),paused=false,shotAim=null;
 const ranges={drive:[1/30,61/30],aim:[81/30,170/30],fire:[181/30,208/30],hatches:[221/30,280/30],shoot:[301/30,385/30]};
 const labels={drive:'履带循环',aim:'火炮瞄准',fire:'后坐复进',hatches:'舱盖开合',explode:'分解观察',shoot:'单次开火'};
 const states={iso:[10,9,12],rear_iso:[-10,8,12],side:[0,0,1],front:[1,0,0],rear:[-1,0,0],top:[0,1,.00001]};
 let baseScale=12, aspect=1;
 function updateCamera(){camera.up.set(0,1,0);camera.position.set(Math.cos(yaw)*Math.cos(elev),Math.sin(elev),Math.sin(yaw)*Math.cos(elev)).multiplyScalar(25).add(target);camera.lookAt(target);camera.updateMatrixWorld();camera.left=-baseScale*aspect/(2*zoom);camera.right=-camera.left;camera.top=baseScale/(2*zoom);camera.bottom=-camera.top;camera.updateProjectionMatrix();}
 ...
 function separation(o){const n=o.name;if(n.startsWith('CASEMATE_')||/^HATCH_[LR]_(PIVOT|RIM)$/.test(n)||n==='REAR_DETAILS')return new T.Vector3(0,.85,0);if(n==='GUN_TRAVERSE_PIVOT')return new T.Vector3(.75,.35,0);if(n==='SUSPENSION_L'||n==='TRACKS_L')return new T.Vector3(0,0,.65);if(n==='SUSPENSION_R'||n==='TRACKS_R')return new T.Vector3(0,0,-.65);if(/^(FRONT_IDLER|REAR_SPROCKET)_[LR]$/.test(n))return new T.Vector3(0,0,n.endsWith('L')?.65:-.65);return new T.Vector3();}
 ...
 function chooseMotion(name){
  shotAim=name==='shoot'?[named.GUN_TRAVERSE_PIVOT.quaternion.clone(),named.GUN_PITCH_PIVOT.quaternion.clone()]:null;
  clock=0;paused=false;wantExplode=0;motion=name==='reset'?'':name;lastMotion=ranges[name]?name:'';shotFX.clear();
  separated.forEach(n=>n.position.copy(rest.get(n)));exploded=0;named.GUN_TRAVERSE_PIVOT.quaternion.identity();named.GUN_PITCH_PIVOT.quaternion.identity();mixer.setTime(0);
  if(shotAim){named.GUN_TRAVERSE_PIVOT.quaternion.copy(shotAim[0]);named.GUN_PITCH_PIVOT.quaternion.copy(shotAim[1]);world.updateMatrixWorld(true);shotFX.capture(named.SOCKET_MUZZLE);}
 ...
 function applyMotion(){if(!ranges[motion])return;const r=ranges[motion],duration=r[1]-r[0];const t=(motion==='shoot'||paused)?Math.min(clock,duration):clock%duration;mixer.setTime(r[0]+t);if(motion==='shoot'&&shotAim){named.GUN_TRAVERSE_PIVOT.quaternion.copy(shotAim[0]);named.GUN_PITCH_PIVOT.quaternion.copy(shotAim[1]);}$('#progress').value=Math.round(t/duration*100);}
 ...
 const points=[[-1.4,2.25,1.43],[-1.52,2.88,.65],[1.24,1.8,.99],[.52,2.27,0],[.445,.56,1.85],[-1.3,.05,1.79]];
 ...
 function updateReadouts(){$('#elevation').textContent=T.MathUtils.radToDeg(named.GUN_PITCH_PIVOT.rotation.z).toFixed(1)+'°';$('#traverse').textContent=T.MathUtils.radToDeg(named.GUN_TRAVERSE_PIVOT.rotation.y).toFixed(1)+'°';$('#hatch').textContent=Math.round(Math.abs(named.HATCH_L_PIVOT.rotation.z)*180/Math.PI/80*100)+'%';$('#recoil').textContent=Math.round(Math.abs(named.GUN_RECOIL_ROOT.position.x)*1000)+' mm';$('#fx-state').textContent=motion!=='shoot'?'待发':clock<.22?'炮口闪焰':clock<1.3?'烟尘扩散':clock<2.6?'烟尘消散':'结束';}
 function render(now){const dt=Math.min((now-last)/1000,.1);last=now;if(!paused)clock+=dt;applyMotion();if(motion==='shoot'&&clock>=2.8&&!paused){clock=2.8;paused=true;$('#pause').textContent='重播';$('#motion').textContent='开火完成';}if(wantExplode||exploded>.001){exploded=T.MathUtils.damp(exploded,wantExplode,5,dt);for(const n of separated)n.position.copy(rest.get(n)).addScaledVector(cachedSep.get(n),exploded);}world.updateMatrixWorld(true);updateInstances();if(motion==='shoot')shotFX.update(clock,named.SOCKET_MUZZLE);renderer.render(world,camera);updateReadouts();
   for(const m of markers){const p=m.p.clone().project(camera);m.el.style.display=view==='iso'&&!motion&&!wantExplode?'grid':'none';m.el.style.left=(p.x*.5+.5)*innerWidth+'px';m.el.style.top=(-p.y*.5+.5)*innerHeight+'px';}
