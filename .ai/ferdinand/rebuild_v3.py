...
scene.render.fps = 30
scene.frame_start, scene.frame_end = 1, 385
...
class Mesh:
    ...
    def lathe_x(self,profile,mat=0,N=64):
        vs=[(x,r*math.cos(i*math.tau/N),r*math.sin(i*math.tau/N)) for x,r in profile for i in range(N)]
        fs=[(j*N+i,j*N+(i+1)%N,((j+1)%len(profile))*N+(i+1)%N,((j+1)%len(profile))*N+i) for j in range(len(profile)) for i in range(N)]
        return self.add(vs,fs,mat,smooth=True)
...
bore(front,(.12,0,2.27),.315,1,(1,0,0))
...
# A fixed, sloped flange surrounds a spherical moving shield. Its bore clears
# the complete yaw/elevation envelope rather than cutting through the armor.
seat=Mesh('CASEMATE_GUN_SEAT'); N=64; rings=[]
for kind,r in (('rear',.400),('front',.365),('front',.310),('rear',.310)):
    ring=[]
    for i in range(N):
        a=i*math.tau/N; y,z=r*math.cos(a),2.27+r*math.sin(a)
        x=(bf+(tf-bf)*(z-bz)/(tz-bz)+.027) if kind=='rear' else .455
        ring.append((x,y,z))
    rings.extend(ring)
seat.add(rings,[(j*N+i,j*N+(i+1)%N,((j+1)%4)*N+(i+1)%N,((j+1)%4)*N+i) for j in range(4) for i in range(N)],0)
seat.ring((.467,0,2.27),.374,.310,.024,1,(1,0,0),64)
seat.bolt_circle((.481,0,2.27),.343,12,(1,0,0),.013); seat.finish(.002)
traverse=empty('GUN_TRAVERSE_PIVOT',root,(.50,0,2.27)); traverse['limits_deg']='-7.5,7.5'
pitch=empty('GUN_PITCH_PIVOT',traverse); pitch['limits_deg']='-4,14'
recoil=empty('GUN_RECOIL_ROOT',pitch)
gun=Mesh('GUN_MANTLET',pitch)
profile=[(x,math.sqrt(.29**2-x*x)) for x in (-.24,-.21,-.17,-.12,-.06,0,.06,.12,.17,.21,.24)]
profile += [(.24,.149),(-.24,.149)]
gun.lathe_x(profile,1)
gun.lathe_x([(.19,.180),(.30,.180),(.57,.157),(.60,.157),(.60,.147),(.19,.147)],0)
gun.ring((.60,0,0),.164,.147,.026,1,(1,0,0)); gun.finish()
...
for side in (-1,1):
    ...
    # Segmented mudguards follow an offset outside the whole moving belt.
    # Their underside stays above the track shoes, including the outer cleats.
    for end,profile in [('FRONT',[(2.875,1.687),(3.18,1.687),(3.46,1.57),(3.65,1.36)]),('REAR',[(-3.36,1.687),(-3.53,1.56),(-3.66,1.38)])]:
        guard=Mesh('FENDER_'+end+'_'+('L' if side<0 else 'R'))
        for (xa,za),(xb,zb) in zip(profile,profile[1:]):
            length=math.hypot(xb-xa,zb-za); angle=-math.atan2(zb-za,xb-xa)
            guard.box(((xa+xb)/2,side*1.52,(za+zb)/2),(length,.68,.034),0,Matrix.Rotation(angle,3,'Y'))
        guard.path([(x,side*1.85,z+.025) for x,z in profile],.015,1)
        guard.finish(.002)
    for xx in (-2.65,1.95):
...
for fr,x in [(1,0),(181,0),(186,-.23),(208,0),(301,0),(303,-.23),(305,-.23),(312,-.13),(322,-.035),(331,0),(385,0)]: recoil.location.x=x; recoil.keyframe_insert(data_path='location',index=0,frame=fr)
for h in hatch_nodes: keyrot(h,1,[(1,0),(221,0),(250,80),(280,0)])
for name,fr in [('DRIVE / track circulation',1),('AIM / limited traverse',81),('FIRE / recoil',181),('HATCHES / inspection',221),('SHOT / flash and smoke',301)]: scene.timeline_markers.new(name,frame=fr)
...
for fr in (1,19,61):
    scene.frame_set(fr); bpy.context.view_layer.update(); deps=bpy.context.evaluated_depsgraph_get()
    solids=[parts[n] for n in ('HULL_LOWER','HULL_SPONSONS','DRIVER_NOSE','CASEMATE_L','CASEMATE_R','FENDERS_AND_TOOLS','FENDER_FRONT_L','FENDER_FRONT_R','FENDER_REAR_L','FENDER_REAR_R')]
...
scene.frame_set(1); bpy.context.view_layer.update()
# Densely sample the newly repaired pairs, including half-frames between keys.
guard_parts=[parts[n] for n in ('FENDERS_AND_TOOLS','FENDER_FRONT_L','FENDER_FRONT_R','FENDER_REAR_L','FENDER_REAR_R')]
guard_trees={o.name:worldtree(o) for o in guard_parts}
for half in range(2,123):
    scene.frame_set(half//2,subframe=(half%2)*.5); bpy.context.view_layer.update()
    for o,s,side in track_links:
        if abs(o.location.x)<2.45: continue
        lt=worldtree(o)
        for name,tree in guard_trees.items():
            if lt.overlap(tree): collision.append({'frame':half/2,'a':o.name,'b':name})
scene.frame_set(1); bpy.context.view_layer.update()
fixed={n:worldtree(parts[n]) for n in ('CASEMATE_FRONT','CASEMATE_GUN_SEAT','ENGINE_DECK','DRIVER_NOSE')}
for yaw in (-7.5,-5,0,5,7.5):
    for elevation in (-4,0,4,9,14):
        for stroke in (0,-.115,-.23):
            traverse.rotation_euler.z=math.radians(yaw); pitch.rotation_euler.y=-math.radians(elevation); recoil.location.x=stroke; bpy.context.view_layer.update()
            for n in ('GUN_MANTLET','GUN_BARREL'):
                moving=worldtree(parts[n])
                for other,tree in fixed.items():
                    if moving.overlap(tree): collision.append({'yaw':yaw,'elevation':elevation,'stroke':stroke,'a':n,'b':other})
checks['mudguard_drive_samples']=121
checks['gun_combined_poses']=75
checks['ball_to_socket_radial_gap_m']=.310-.290
scene.frame_set(1); bpy.context.view_layer.update()
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'ferdinand_technical.blend'))
import runpy
runpy.run_path(str(Path(__file__).with_name('add_shot_fx.py')),run_name='__main__')
